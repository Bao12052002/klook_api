...
class Product extends Model {
    ...
    public function getProductOptions($productId) {
        $sql = "SELECT * FROM product_options WHERE product_id = ? ORDER BY is_default DESC";
    ...
    public function getOptionUnits($productId, $optionId) {
        $sql = "SELECT * FROM units WHERE product_id = ? AND option_id = ? AND status = 'active' ORDER BY type";
    ...
    public function updateWithOptions($id, $data) {
        try {
            ...
            if ($options !== null) {
                ...
                $this->query("DELETE FROM units WHERE product_id = ?", [$id]);
    ...
    private function createProductOption($productId, $optionData) {
        ...
        // Create option
        $sql = "INSERT INTO product_options (" . implode(', ', array_keys($optionData)) . ") VALUES (:" . implode(', :', array_keys($optionData)) . ")";
    ...
    private function createOptionUnit($productId, $optionId, $unitData) {
        ...
        // Create unit
        $sql = "INSERT INTO units (" . implode(', ', array_keys($unitData)) . ") VALUES (:" . implode(', :', array_keys($unitData)) . ")";
