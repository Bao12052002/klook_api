...
class BookingController extends Controller {
    ...
    public function reserveBooking() {
        try {
         ...
         $bookingResponse = $this->bookingModel->getBookingForKlookResponse(
    $dbBookingId,                 // Tham số #1: ID booking nội bộ (string)
    new ProductController(),      // Tham số #2: ProductController instance
    $this->productModel,          // Tham số #3: Product model instance
    $this->productOptionModel,    // Tham số #4: ProductOption model instance
    $this->availabilityModel,     // Tham số #5: Availability model instance
    false                         // Tham số #6: isKlookUUID (false vì $dbBookingId là ID nội bộ)
);// false vì dùng ID nội bộ
            if(!$bookingResponse) {
